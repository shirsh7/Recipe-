//const Create = require('./models/Create'); // Import the Recipe model
...
res.json({message:"Login successful",username:user.username, userId: user._id});//if found show msg Login successful and and username of that user
}
catch(err){
    console.log(err)
}
...
app.post("/create-recipe", async (req, res) => {
    const { userId, name, description, ingredients, instructions, imageUrl, cookingTime } = req.body;
    ...
    try {
        const newRecipe = new Recipe({ userId, name, description, ingredients, instructions, imageUrl, cookingTime });
